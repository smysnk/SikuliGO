...
- Optional OCR tagged job:
  - Runs `go test -tags gosseract ./internal/ocr ./pkg/sikuli` in a non-blocking (`continue-on-error`) job.
  - Installs Tesseract/Leptonica runtime dependencies on Ubuntu runners before tagged OCR execution.
- Parity end-to-end job:
  - Runs cross-protocol parity flows in `pkg/sikuli` for default and `-tags gosseract` builds.
...
go test -tags gosseract -run '^TestCrossProtocolIntegrationFlowTaggedOCR$' ./pkg/sikuli
```
